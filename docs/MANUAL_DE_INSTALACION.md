...
> **Versión del manual:** 1.1 · **App:** v1.0.0 · **Branch de referencia:** `main`
...
| **Node.js** | Versión **22.9 o superior** (el instalador lo descarga e instala solo si no existe, vía `winget`). Se requiere 22.9+ porque el server carga `.env` con `--env-file-if-exists` (flag nativo) | ✅ automático |
...
> 💡 **¿Cómo saber si Node.js ya está instalado?** Abre una ventana de comandos (tecla Windows → escribe `cmd` → Enter) y escribe: `node -v`. Si te muestra una versión **`v22.9.x` o superior** (ej. `v22.x.x`), está listo. Si dice "no se reconoce", el instalador lo pondrá solo.
...
NODE_ENV=production                        # ← production en el bar
...
| `NODE_ENV` | `production` en el bar (desactiva stack traces en errores y **exige** `JWT_SECRET`); `development` en la máquina de trabajo |
| `JWT_SECRET` | **Secreto de seguridad** que firma los inicios de sesión. **Mínimo 32 caracteres.** En `NODE_ENV=production` su ausencia **aborta el arranque** (fail-loud, ver abajo) |
...
> **Variables de entorno obligatorias (resumen):** el server arranca con
> `node --env-file-if-exists=.env server/index.js` (requiere Node ≥ 22.9).
> En producción, `JWT_SECRET` es **obligatorio**: si `NODE_ENV=production` y
> falta, el guard `server/config/env-guard.js` **aborta el arranque** antes de
> escuchar (Art. VI Observabilidad: fail loud, never silent). Nunca dejes un
> server productivo con el fallback de desarrollo. Otras variables opcionales
> leídas por el server: `DB_PATH`, `AUTH_RATE_LIMIT_MAX`, `CORS_ORIGINS`,
> `JWT_EXPIRES_IN`, `LOG_DIR`, `DEFAULT_TABLES` (debe coincidir con
> `capacity.totalTables` del SSOT — 10).
...
> 🔒 Si `JWT_SECRET` está vacío o no existe en **desarrollo**, el sistema arranca igual pero con un secreto de **desarrollo** (aparece una advertencia en consola). En **producción** (`NODE_ENV=production`) el arranque **se aborta** — eso es intencional (fail-loud).
...
|------|----------|:---:|---------------------|
| **[1/7] Prerequisitos** | Verifica/instala Node.js 22.9+, npm y Git (con `winget` si faltan) | 10 s – 3 min | Mensajes `[OK]` para los tres |
...
| **El server no arranca y dice `JWT_SECRET` requerido** | `NODE_ENV=production` sin `JWT_SECRET` en `.env` | Pon `NODE_ENV` en `development` (máquina de trabajo) o define `JWT_SECRET` con 32+ caracteres (genera uno con `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"`). Es un **fail-loud intencional**, no un bug |
...
| **Build falla** (fase 5/7) | Dependencias incompletas, o Node.js muy antiguo | Revisa que Node sea **22.9+** (`node -v`). Reinstala dependencias: `npm install --legacy-peer-deps` y repite `npm run build`. Lee el error en pantalla (copia el mensaje rojo) |
