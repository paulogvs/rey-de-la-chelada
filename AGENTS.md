...
| **Promos data-driven (v15 2026-08-29)** | Promos y extras ya NO viven solo en código: Admin los gestiona en **Promos** (armador items/grupos + precio + toggle + días de semana/rango) y **Extras** (por grupo del menú). Schema v15 (`promos`, `promo_lines`, `promo_schedule`, `category_extras`). Server: `promos-service.js`/`extras-service.js` + CRUD `/api/promotions/admin` y `/api/extras/:cat`. `GET /api/promotions` fusiona DB+SSOT. `order-pricing.js` valida packs DB (max_per_order + líneas). Meseros consumen `useActivePromos` (API + fallback SSOT); `PromotionsToday` (caja/admin) también consume la API. `sync.js` valida contexto de promos en push offline. Cambios de promos/extras → `menu_changed` (debounce 1s) → refetch en todos los PWAs. |
