...
| **Sync push** | `server/routes/sync.js`: idempotente por UUID cliente (`orders.id`/`payments.id` → `skipped` + `duplicate_*_already_exists`). Errores parciales → HTTP 200, `success:false`, `code:'SYNC_PARTIAL_ERRORS'`. **Estado real del offline:** el cliente `SyncEngine` (`src/core/engine/SyncEngine.ts` + SyncQueue IndexedDB) está implementado y testeado, pero **NO está cableado a la UI** — los meseros operan online hoy (los POST van directo a /api/orders). El server ya soporta el contrato; cablear la UI (cola offline + aviso de conflictos) es mejora futura, NO bloquea la apertura (local con WiFi). |
