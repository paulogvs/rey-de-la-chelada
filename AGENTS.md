...
> Actualizado: 2026-09-02 — v18 (menú sin Promociones, item disponible, modal apartado/item) + v19 (export HTML/WhatsApp en Reportes, gráfico barras en Estadísticas, layout topbar + scroll PC)
...
| **Promos data-driven (v15 2026-08-29)** | Promos y extras gestionados 100% desde Admin: **Promos** (armador items/grupos + precio + **toggle ON/OFF**, sin programador de días — se activan/desactivan a mano) y **Extras por grupo** (botón "Agregar extra" en Menú → subgrupo en la card). Schema v15 (`promos`, `promo_lines`, `promo_schedule`, `category_extras`). **La DB es la ÚNICA fuente** — `GET /api/promotions` devuelve solo las activas de la DB (el SSOT NO se fusiona desde v15 2026-08-31; solo siembra la DB vía `seedDefaultPromos` idempotente con `active=0`; Shot/Escarchado → extras de Micheladas). Meseros consumen `useActivePromos` (API, inicializa vacío, fallback SSOT solo si la tabla no existe). Crear apartado/item/extras → modal con diseño app; apartados vacíos visibles. Cambios → `menu_changed` (debounce 1s, **ya se emite explícitamente en CADA mutador de `server/routes/menu.js`**) → refetch en todos los PWAs. |
| **Área del grupo (v17 2026-09-01)** | `menu_categories.area` (`bar`\|`cocina`) — el área se define en el **GRUPO** (Admin → modal apartado → selector "Área (KDS)") y se **HEREDA** a todos sus items ("**FORZAR TODO**": al crear/cambiar el área de un grupo se hace `UPDATE menu_items SET area = <área>`; al crear un item NO se elige área — el server la deriva de `category_id`). Migración v17 ADITIVA (`ADD COLUMN` + `hasColumn`): categorías existentes infieren el área de su **primer item** (o `cocina` si está vacía) y todos sus items se fuerzan al área del grupo. El KDS lee `menu_items.area` → el grupo decide el módulo (Barra/Cocina). `utils/menuAreas.ts` prioriza `category.area` (fallback al primer item). **Tab PROMOS de meseros (OrderPanel)** = listado de promos **ACTIVAS** de la DB (`useActivePromos`); las promos se agregan **solo desde ahí** (se quitó el `PromosCollapsible` del carrito y del `CartModal`). `SCHEMA_VERSION = 17`. |
| **Menú v18 (2026-09-01)** | La categoría **"Promociones" SE QUITÓ del `menu-seed.json`** (las promos se manejan SOLO desde el armador — tablas `promos`/`promo_lines`/`promo_schedule`). El seed queda con **99 items / 18 categorías** (12 BAR 🍻 / 6 COCINA 🍽️). **Modal de nuevo apartado:** solo **Nombre + Área (Barra/Cocina)** — el icono es **automático** (**🍻 Barra / 🍽 Cocina**). **Modal de nuevo item:** solo **Nombre + Precio inicial (Bs)** — el apartado viene por defecto. **Item nuevo disponible al instante:** al crear un item el server fuerza `is_available=1` (salvo indicación explícita) → **aparece al instante en meseros** (antes quedaba oculto por `is_available=0`). `load-menu.js` ahora carga `menu_categories.area` al importar el seed (fix: antes todas quedaban `cocina`). |
| **Export de Reportes v19 (2026-09-02)** | Admin → **Reportes** (día seleccionado en el picker global) → **"Exportar HTML"** genera un `.html` imprimible (estética dorado/oscuro) que se guarda como PDF con `Ctrl+P` → "Guardar como PDF"; incluye **cierre del día + pedidos/ventas + top más vendido** (cantidad y monto, con barras). **"Copiar WhatsApp"** copia un **texto resumido** del día para pegar en WhatsApp. Util: `src/pwa/_shared/utils/reportExport.ts` (`buildReportHtml` / `buildWhatsAppText` / `downloadReportHtml`). Datos de APIs existentes (`sales/daily`, `items/popular`, `closings`). **Estadísticas (v19):** "Top productos por CANTIDAD" muestra **gráfico de barras doradas** (CSS puro, top 5). |
| **Export de Reportes v19 (2026-09-02)** | Icono `message` agregado a `AppIcon` (lucide `MessageSquare`) para el botón "Copiar WhatsApp". Layout: Admin topbar sticky sin sobreponerse al contenido; Meseros → scroll horizontal visible en PC (`order-panel__categories`, media query ≥1025px). |
